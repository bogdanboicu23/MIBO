...
export function FormCard({ props, data, onAction }: UiComponentProps) {
    ...
    const title = String(rawProps.title ?? "Form");
    const subtitle = String(rawProps.subtitle ?? "");
    const actionType = resolveActionType(rawProps, "ui.form.submit");

    const fields = useMemo(() => normalizeFields(rawProps.fields), [rawProps.fields]);

    const initialFromProps = useMemo(
        () => extractObject(rawProps.initialValues) ?? {},
        [rawProps.initialValues]
    );
    const initialDataKey = String(rawProps.initialDataKey ?? rawProps.dataKey ?? "").trim();
    const initialFromData = useMemo(() => {
        if (!initialDataKey) return {};
        return extractObject(resolveFromDataKey((data ?? {}) as Record<string, any>, initialDataKey)) ?? {};
    }, [data, initialDataKey]);
    ...
    useEffect(() => {
        setForm((current) => (areRecordsEqual(current, computedInitialValues) ? current : computedInitialValues));
